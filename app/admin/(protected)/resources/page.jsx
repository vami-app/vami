...
import { Pencil, Trash2, Plus, FileText, UploadCloud, ExternalLink } from 'lucide-react';
...
export default function ResourcesAdminPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(formatApiError(data, 'Failed to load resources'));
  ...
  async function confirmDelete() {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/resources?id=${encodeURIComponent(deleteTarget.id)}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="flex flex-col flex-1 w-full min-h-0">
      ...
      <ConfirmModal
        ...
        description={`Delete "${deleteTarget?.name || 'this resource'}"? This cannot be undone.`}
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between animate-in fade-in slide-in-from-top-4 duration-700 ease-out">
        <div className="min-w-0">
          ...
          <p className="text-sm text-text-muted mt-1 max-w-xl">
            Catalogues, TDS, and company profile PDFs for the downloads area.
          </p>
        </div>
          ...
          {visible.length === 0 && (
            <div className="text-center py-16 px-4">
              ...
              <h3 className="text-lg font-medium text-text-primary">
                {items.length === 0 ? 'No resources yet' : 'No matches for this filter'}
              </h3>
              <p className="mt-1 text-sm text-text-muted max-w-sm mx-auto">
                {items.length === 0
                  ? 'Upload catalogues and TDS as draft until ready to publish.'
        ...
        <form onSubmit={handleSave} className="space-y-5">
          <div>
            ...
            <Input
              ...
              placeholder="e.g. Aluminium Circles Catalogue 2026"
            />
          </div>
          <div>
            <Label className="mb-2 ml-1">Type</Label>
            <Select
              value={form.type}
          ...
          <div>
            <Label className="mb-2 ml-1">PDF file</Label>
