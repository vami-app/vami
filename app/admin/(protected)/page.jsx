...
import { Package, FileText, FolderTree, Inbox } from 'lucide-react';
import Link from 'next/link';

export default async function AdminDashboard() {
  const { productCount, blogCount, categoryCount, leadNewCount, leadTotal } =
    await getDashboardStats();

  const stats = [
    { name: 'New Leads', stat: leadNewCount ?? 0, icon: Inbox, href: '/admin/leads' },
    { name: 'Total Leads', stat: leadTotal ?? 0, icon: Inbox, href: '/admin/leads' },
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <h1 className="text-3xl sm:text-4xl font-headline font-light text-text-primary mb-8 tracking-tight">
        Dashboard Overview
      </h1>
          ...
          <Link key={item.name} href={item.href}>
            <div className="relative bg-surface p-6 sm:p-8 border border-border-subtle rounded-[calc(var(--outer-radius)-8px)] overflow-hidden hover:shadow-md hover:-translate-y-1 transition-all duration-300 group flex items-start justify-between">
              <div>
                ...
                <p className="mt-4 text-4xl sm:text-5xl font-headline font-light text-text-primary tracking-tight">
                  {item.stat}
                </p>
              </div>
              <div className="bg-surface-muted rounded-lg p-4 border border-border-subtle group-hover:bg-primary group-hover:border-primary transition-colors duration-300 flex items-center justify-center">
                <item.icon
                  className="h-6 w-6 text-text-muted group-hover:text-primary-foreground transition-colors duration-300"
                  aria-hidden="true"
                />
              </div>
            </div>
