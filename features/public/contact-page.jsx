              ...
              <div className={`space-y-8 border-t border-border-subtle pt-10 transition-all duration-1000 delay-200 ease-out transform ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                ...
                <div className="flex items-start group">
                  ...
                  <div className="ml-6">
                    ...
                    <p className="mt-2 text-text-muted font-light leading-relaxed">
                      <a href="mailto:radhemetalalloysllp@gmail.com" className="hover:text-text-primary transition-colors underline underline-offset-4 decoration-border-base hover:decoration-text-primary">radhemetalalloysllp@gmail.com</a>
                    </p>
                  </div>
                </div>
            ...
            <div className={`lg:pl-8 transition-all duration-1000 delay-300 ease-out transform ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
              <div className="bg-surface rounded-[var(--outer-radius)] p-8 sm:p-10 border border-border-subtle shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgb(0,0,0,0.06)] transition-shadow duration-500">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label htmlFor="company" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Company Name & Contact Person</label>
                    <input type="text" name="company" id="company" required className="block w-full py-3.5 px-5 bg-surface/50 border border-border-subtle rounded-2xl focus:bg-surface focus:ring-1 focus:ring-text-primary focus:border-text-primary transition-all duration-300 hover:border-border-base outline-none text-text-primary shadow-[0_2px_10px_rgba(0,0,0,0.02)]" placeholder="Acme Industries - Jane Doe" />
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label htmlFor="email" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Email Address</label>
                      <input type="email" name="email" id="email" required className="block w-full py-3.5 px-5 bg-surface/50 border border-border-subtle rounded-2xl focus:bg-surface focus:ring-1 focus:ring-text-primary focus:border-text-primary transition-all duration-300 hover:border-border-base outline-none text-text-primary shadow-[0_2px_10px_rgba(0,0,0,0.02)]" placeholder="jane@acme.com" />
                    </div>
                    <div>
                      <label htmlFor="phone" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Mobile Number</label>
                      <input type="tel" name="phone" id="phone" required className="block w-full py-3.5 px-5 bg-surface/50 border border-border-subtle rounded-2xl focus:bg-surface focus:ring-1 focus:ring-text-primary focus:border-text-primary transition-all duration-300 hover:border-border-base outline-none text-text-primary shadow-[0_2px_10px_rgba(0,0,0,0.02)]" placeholder="+91 00000 00000" />
                    </div>
                  </div>
                  
                  {/* Metal Category Dropdown */}
                  <div>
                    <label htmlFor="category" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Metal Category</label>
                    ...
                    <div className={`relative ${isCategoryOpen ? 'z-30' : 'z-10'}`}>
                      ...
                      >
                        <span className="text-text-primary font-medium">{selectedCategory}</span>
                  ...
                  <div>
                    <label htmlFor="alloy" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Alloy Grade Required</label>
                    <input type="text" name="alloy" id="alloy" required className="block w-full py-3.5 px-5 bg-surface/50 border border-border-subtle rounded-2xl focus:bg-surface focus:ring-1 focus:ring-text-primary focus:border-text-primary transition-all duration-300 hover:border-border-base outline-none text-text-primary shadow-[0_2px_10px_rgba(0,0,0,0.02)]" placeholder="e.g., ETP, C260, C52100" />
                  </div>

                  {/* Form Factor Dropdown */}
                  <div>
                    <label htmlFor="factor" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Form Factor</label>
                    
                    {isFactorOpen && (
                      <div className="fixed inset-0 z-10" onClick={() => setIsFactorOpen(false)}></div>
                    )}
                    
                    <div className={`relative ${isFactorOpen ? 'z-30' : 'z-10'}`}>
                      <input type="hidden" name="factor" value={selectedFactor} />
                      <button 
                        type="button" 
                        onClick={() => setIsFactorOpen(!isFactorOpen)}
                        className={`w-full py-3.5 px-5 bg-surface/50 border ${isFactorOpen ? 'border-text-primary ring-1 ring-text-primary bg-surface' : 'border-border-subtle hover:border-border-base'} rounded-2xl transition-all duration-300 outline-none text-left flex justify-between items-center shadow-[0_2px_10px_rgba(0,0,0,0.02)]`}
                      >
                        <span className="text-text-primary font-medium">{selectedFactor}</span>
                        <svg className={`h-4 w-4 text-text-muted transition-transform duration-300 ease-out ${isFactorOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                      </button>
                      
                      <div className={`absolute top-[calc(100%+8px)] left-0 w-full bg-surface/95 backdrop-blur-xl border border-border-base rounded-2xl shadow-[0_16px_40px_rgba(0,0,0,0.08)] overflow-hidden p-2 z-30 transition-all duration-300 ease-[cubic-bezier(0.2,0.7,0.3,1)] ${isFactorOpen ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-2 scale-[0.98] pointer-events-none'}`}>
                        <div className="space-y-1">
                  ...
                  <div>
                    <label htmlFor="dimensions" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Dimensions / Blueprints Required</label>
                    <textarea id="dimensions" name="dimensions" rows={2} required className="block w-full py-3.5 px-5 bg-surface/50 border border-border-subtle rounded-2xl focus:bg-surface focus:ring-1 focus:ring-text-primary focus:border-text-primary transition-all duration-300 hover:border-border-base outline-none text-text-primary resize-y shadow-[0_2px_10px_rgba(0,0,0,0.02)]" placeholder="Thickness, Width, Diameter..."></textarea>
                  </div>

                  <div>
                    <label htmlFor="quantity" className="block text-sm font-medium text-text-secondary mb-2 ml-1">Order Quantity</label>
                    <input type="text" name="quantity" id="quantity" required className="block w-full py-3.5 px-5 bg-surface/50 border border-border-subtle rounded-2xl focus:bg-surface focus:ring-1 focus:ring-text-primary focus:border-text-primary transition-all duration-300 hover:border-border-base outline-none text-text-primary shadow-[0_2px_10px_rgba(0,0,0,0.02)]" placeholder="In Kilograms or Metric Tons" />
                  </div>

                  <div className="pt-2 px-1">
                    <label className="block text-sm font-medium text-text-secondary mb-5">Testing Documentation Required?</label>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <label className="flex items-center space-x-3 p-4 border border-border-subtle rounded-xl hover:bg-surface-subtle cursor-pointer transition-colors group">
                        <input type="checkbox" className="form-checkbox h-5 w-5 text-text-primary rounded border-border-subtle focus:ring-text-primary bg-surface/50 group-hover:bg-surface" />
                        <span className="text-sm font-medium text-text-primary">TC</span>
                      </label>
                      <label className="flex items-center space-x-3 p-4 border border-border-subtle rounded-xl hover:bg-surface-subtle cursor-pointer transition-colors group">
                        <input type="checkbox" className="form-checkbox h-5 w-5 text-text-primary rounded border-border-subtle focus:ring-text-primary bg-surface/50 group-hover:bg-surface" />
                        <span className="text-sm font-medium text-text-primary">NABL</span>
                      </label>
                      <label className="flex items-center space-x-3 p-4 border border-border-subtle rounded-xl hover:bg-surface-subtle cursor-pointer transition-colors group">
                        <input type="checkbox" className="form-checkbox h-5 w-5 text-text-primary rounded border-border-subtle focus:ring-text-primary bg-surface/50 group-hover:bg-surface" />
                        <span className="text-sm font-medium text-text-primary">Ultrasonic</span>
                      </label>
                    </div>
                  </div>
                  
                  <div className="pt-6">
                    <button type="submit" disabled={loading} className="w-full flex items-center justify-center py-4 px-8 border border-transparent rounded-full shadow-md hover:shadow-lg text-base font-medium text-text-inverse bg-text-primary hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-text-primary transition-all disabled:opacity-70 disabled:cursor-not-allowed group">
                      {loading ? (
                        <>
