...
export function CategoryPageFeature({ category, products }) {
  return (
    <PageShell>
              ...
              {products.map((product) => (
                <Link key={product._id} href={`/products/${category.slug}/${product.slug}`} className="relative group block">
                  <div className="w-full aspect-[5/4] sm:aspect-square rounded-[var(--inner-radius)] overflow-hidden bg-surface-muted border border-border-subtle relative shadow-sm">
                    ...
                    {/* Top Badge */}
                    {product.category && (
                      <div className="absolute top-3 left-3 max-w-[calc(100%-1.5rem)] truncate block bg-surface/90 backdrop-blur-md px-3 py-1.5 rounded-full text-[10px] font-semibold text-text-primary uppercase tracking-wider shadow-sm border border-border-subtle z-10" title={category.name}>
                        {category.name}
                      </div>
                    )}
