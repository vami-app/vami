...
import { Contact, X, Mail, FileText } from "lucide-react";
...
export default function FloatingContactButton({
  email = "radhemetalalloysllp@gmail.com",
  whatsappNumber = "919081358107",
}) {
  const [isOpen, setIsOpen] = useState(false);
  const wa = String(whatsappNumber).replace(/[^\d]/g, "") || "919081358107";
      ...
      <div className="w-full max-w-[var(--max-width-layout)] relative pointer-events-none">
        <div className="absolute right-[var(--gap)] bottom-0 flex flex-col items-center gap-4 pointer-events-auto">
          ...
          >
            ...
            >
              <FileText className="h-5 w-5 sm:h-6 sm:w-6 text-text-primary" />
            </Link>
          ...
          >
            {isOpen ? <X className="h-6 w-6 sm:h-7 sm:w-7" /> : <Contact className="h-6 w-6 sm:h-7 sm:w-7" />}
          </button>
        </div>
      </div>
    </div>
